...
/**
 * @author hesham
 *	this class extends spring social's connect controller just to override some of its default 
 *	functionalities and render model into different views
 */
...
//	@Inject
	...
	public String connectionStatus(NativeWebRequest request, Model model) {
		...
		return appropriateView(request, DEFAULT_BODIES_PREFIX+connectView(), defaultView(model, connectView()));
	}
	...
	public String connectionStatus(@PathVariable String providerId, NativeWebRequest request, Model model) {
		...
		if (connections.isEmpty()) {
			return connectView(model,providerId,request); 
		} else {
			model.addAttribute("connections", connections);
			return connectedView(model,providerId,request);
		}
	}
	 ...
	 */
	protected String connectView(Model model,String providerId,HttpServletRequest request) {
		return appropriateView(request,  DEFAULT_BODIES_PREFIX+getViewPath()+providerId+"Connect", defaultView(model, getViewPath() + providerId + "Connect"));
	}
	
	protected String connectView(Model model,String providerId,NativeWebRequest request) {
		return appropriateView(request,  DEFAULT_BODIES_PREFIX+getViewPath()+providerId+"Connect", defaultView(model, getViewPath() + providerId + "Connect"));
	}
	 ...
	 * @param providerId the ID of the provider to display the connection status for.
	 */
	protected String connectedView(Model model,String providerId,NativeWebRequest request) {
		return appropriateView(request, DEFAULT_BODIES_PREFIX+getViewPath()+providerId+"Connected", defaultView(model, getViewPath() + providerId + "Connected"));
	}
	protected String connectedView(Model model,String providerId,HttpServletRequest request) {
		return appropriateView(request, DEFAULT_BODIES_PREFIX+getViewPath()+providerId+"Connected", defaultView(model, getViewPath() + providerId + "Connected"));
	}
